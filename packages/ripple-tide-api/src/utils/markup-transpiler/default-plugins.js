...
const pluginButton = function () {
    ...
    this.find('.button').map((i, el) => {
        ...
        const button = `<rpl-button ${attributes.join(' ')}>${buttonText}</rpl-button>`;
...
const pluginEmbeddedDocument = function () {
    this.find('.embedded-entity--media--file, .embedded-entity--media--document, .embedded-entity .media--type-document').map((i, element) => {
      ...
      <figure class="rpl-markup__document-link">
        <rpl-text-link class="rpl-markup__document-link-link" aria-label="${ariaLabel}" link="${url}" :icon="false" :underline="false" download="${isExternalLink ? 'false' : ''}" target="_blank">
          ${icon
          ...
          <div class="rpl-markup__document-link-info">
            ...
            <div class="rpl-markup__document-link-meta">
              ${extension
                ...
                : ''}
              ${filesize
                ? `<span class="rpl-markup__document-link-size${extension && filesize
        ...
        ${caption
                ? `<figcaption class="rpl-markup__document-link-caption">${_escapeQuotes(caption)}</figcaption>`
                : ''}
      </figure>
      `;
...
const pluginImage = function () {
    ...
    this.find('.embedded-entity--media--image').map((i, el) => {
        ...
        if ($container) {
            ...
            return $container.replaceWith(`<div class="rpl-markup__image"><rpl-responsive-img src="${src}" alt="${alt}" :height="${height}" fit="contain" :blur="true" :srcset="[375, 696, 696, 1200]" width="818" ></rpl-responsive-img></div>`);
        }
    });
};
...
const pluginLinks = function () {
    this.find('a').map((i, el) => {
        ...
        const a = `<rpl-text-link ${attributes.join(' ')}>${_escapeQuotes(text)}</rpl-text-link>`;
