...
<template>
  <svg
    v-if="icon || legacyIcon"
    :class="iconClass"
    :style="iconStyle"
    :viewBox="box"
    :aria-hidden="iconAttribute[0]"
    overflow="visible"
  >
    <title v-if="iconAttribute[1]">{{ iconAttribute[1] }}</title>
    <template v-if="legacyIcon">
      ...
      <use
        :xlink:href="'#' + iconPrefix + symbol"
        :href="'#' + iconPrefix + symbol"
      ></use>
    </template>
    <template v-else>
      ...
      <template v-if="icon.polygons">
        <polygon
          v-for="(polygon, i) in icon.polygons"
          :key="`${symbol}-polygon${i}`"
          v-bind="polygon"
        />
      </template>
    </template>
