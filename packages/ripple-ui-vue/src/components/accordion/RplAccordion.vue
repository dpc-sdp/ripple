...
import {
  RplAccordionProps,
  RplAccordionItemProps
} from '@dpc-sdp/ripple-ui-shared/contracts'

const props = withDefaults(defineProps<RplAccordionProps>(), {
  items: () => [],
  numbered: false,
...
sharedActiveItems.value.push(
  ...props.items.reduce(
    (result: string[], current: RplAccordionItemProps): string[] => {
      if (current.active) {
...
<style src="@dpc-sdp/ripple-ui-styles/components/accordion/RplAccordion.css" />
