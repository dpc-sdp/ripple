<template>
  ...
  <figure v-if="body.image" class="tide-news__image rpl-u-margin-none">
    <RplImage v-if="body.image" :src="body.image.src" :alt="body.image.alt" :width="body.image.width"
      :height="body.image.height" data-cy="featured-image" />
    <figcaption v-if="body.caption" class="rpl-type-p rpl-u-margin-t-2">
      {{ body.caption }}
    </figcaption>
  </figure>
  <RplContent :html="body.content" class="tide-news__content rpl-u-margin-t-4" />
</template>
