import { test, expect } from '@playwright/experimental-ct-vue'
import RplMediaGallery from './RplMediaGallery.vue'

const items = [
  {
    title: 'One',
    alt: 'Alt text',
    thumbnail:
      'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA1',
    image:
      'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA1'
  },
  {
    title: 'Two',
    alt: 'Alt text',
    thumbnail:
      'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7',
    image:
      'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7'
  }
]

const props = { id: 'gallery-1', items }

test.describe('RplMediaGallery', () => {
  test('mounts', async ({ mount }) => {
    await mount(RplMediaGallery, { props })
  })

  test('display a gallery fullscreen', async ({ mount, page }) => {
    const component = await mount(RplMediaGallery, { props })

    await component
      .locator('.rpl-media-gallery__primary-content .rpl-slider__slide')
      .first()
      .locator('.rpl-media-gallery__button')
      .click()

    await expect(page.locator('.rpl-media-gallery__modal')).toBeVisible()
  })

  test('navigates to through items with pagination', async ({ mount }) => {
    const component = await mount(RplMediaGallery, { props })

    await component.locator('[aria-label="Go to next item"]').click()
    await expect(
      component.locator(
        '.rpl-media-gallery__primary-content .swiper-slide-active'
      )
    ).toContainText(items[1].title)
    await expect(
      component.locator(
        '.rpl-media-gallery__primary-images .swiper-slide-active .rpl-image'
      )
    ).toHaveAttribute('src', items[1].thumbnail)

    await component.locator('[aria-label="Go to previous item"]').click()
    await expect(
      component.locator(
        '.rpl-media-gallery__primary-content .swiper-slide-active'
      )
    ).toContainText(items[0].title)
    await expect(
      component.locator(
        '.rpl-media-gallery__primary-images .swiper-slide-active .rpl-image'
      )
    ).toHaveAttribute('src', items[0].thumbnail)
  })

  test('displays the item navigated too fullscreen', async ({
    mount,
    page
  }) => {
    const component = await mount(RplMediaGallery, { props })

    await component.locator('[aria-label="Go to next item"]').click()

    await component
      .locator('.rpl-media-gallery__primary-content .swiper-slide-active')
      .locator('.rpl-media-gallery__button')
      .click()

    const modal = page.locator('.rpl-media-gallery__modal')
    await expect(modal).toBeVisible()

    await expect(
      modal.locator('.rpl-media-gallery__modal-content .swiper-slide-active')
    ).toContainText(items[1].title)
    await expect(
      modal.locator(
        '.rpl-media-gallery__modal-images .swiper-slide-active .rpl-image'
      )
    ).toHaveAttribute('src', items[1].thumbnail)
  })

  test('allows navigating through items in fullscreen gallery', async ({
    mount,
    page
  }) => {
    const component = await mount(RplMediaGallery, { props })

    await component
      .locator('.rpl-media-gallery__primary-content .rpl-media-gallery__button')
      .first()
      .click()

    const modal = page.locator('.rpl-media-gallery__modal')

    await modal.locator('[aria-label="Go to next item"]').click()

    await expect(
      modal.locator('.rpl-media-gallery__modal-content .swiper-slide-active')
    ).toContainText(items[1].title)
    await expect(
      modal.locator(
        '.rpl-media-gallery__modal-images .swiper-slide-active .rpl-image'
      )
    ).toHaveAttribute('src', items[1].image)

    await modal.locator('[aria-label="Go to previous item"]').click()

    await expect(
      modal.locator('.rpl-media-gallery__modal-content .swiper-slide-active')
    ).toContainText(items[0].title)
    await expect(
      modal.locator(
        '.rpl-media-gallery__modal-images .swiper-slide-active .rpl-image'
      )
    ).toHaveAttribute('src', items[0].image)
  })
})
