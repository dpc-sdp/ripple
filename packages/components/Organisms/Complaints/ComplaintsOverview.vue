<template>
  <div class="rpl-complaints-overview">
    <rpl-list class="rpl-complaints-overview__list" size="large" v-if="title || list" :title="title" :link="!!listing && link && link.url ? link.url : null" :list="list" />
    <rpl-markup class="rpl-complaints-overview__description" v-if="description" :html="description"></rpl-markup>
    <rpl-button class="rpl-complaints-overview__cta" v-if="!listing && link" :href="link.url" theme="primary">{{ link.text }}</rpl-button>
  </div>
</template>
...
<style lang="scss">
  ...
  $rpl-complaints-overview-description-ruleset: ('s', 1.5em, 'regular') !default;
  $rpl-complaints-overview-description-color: rpl-color('extra_dark_neutral') !default;
  $rpl-complaints-overview-title-margin: 0 0 ($rpl-space * 7 - $rpl-space-3) 0 !default;
  $rpl-complaints-overview-list-margin: 0 0 ($rpl-space * 6) 0 !default;
  $rpl-complaints-overview-description-margin: 0 0 ($rpl-space * 8) 0 !default;

  .rpl-complaints-overview {

    &__list {
      margin: $rpl-complaints-overview-title-margin;

      &.rpl-list--large {
        .rpl-list__title {
          margin: $rpl-complaints-overview-list-margin;
        }
      }
      a.rpl-list__title {
        display: block;
    ...
    &__description {
      @include rpl_typography_ruleset($rpl-complaints-overview-description-ruleset);
      @include rpl_text_color($rpl-complaints-overview-description-color);
      margin: $rpl-complaints-overview-description-margin;
    }
  }
</style>
