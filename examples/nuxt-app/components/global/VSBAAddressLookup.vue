...
function onAddressSearch(payload: any) {
  ...
  if (payload.type === 'locality') {
    queryAreaAndCenter(
      { where: `gazetted_locality_name='${payload.name.toUpperCase()}'` },
      '11'
    )
  }
  if (payload.type === 'postcode') {
    queryAreaAndCenter({ where: `postcode=${payload.name}` })
  }
...
async function queryAreaAndCenter(query, layer = '14') {
  const serviceUrl = `https://services6.arcgis.com/GB33F62SbDxJjwEL/arcgis/rest/services/Vicmap_Admin/FeatureServer`
  const format = 'json'
  const outSR = '3857'
  ...
  if (arcGisResponse.features && arcGisResponse.features.length === 1) {
    const coords = arcGisResponse.features[0].centroid
    centerMap([coords.x, coords.y])
  }
}
